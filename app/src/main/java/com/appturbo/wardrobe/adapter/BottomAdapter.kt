...
class BottomAdapter(val onFavouriteFoundListener: OnFavouriteFoundListener) :
    ...
    private fun onBind(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            is TopViewHolder -> {
                TopAdapter.mFavouriteFromBottom = mDataList.get(position).id
